...
{
    add_action('plugins_loaded', 'base_init', 0);
    add_filter('woocommerce_payment_gateways', 'base_add_gateway' );
}

function base_add_gateway( $methods ) { $methods[] = 'WC_Gateway_base'; return $methods; }

function base_init() {
  class WC_Gateway_base extends WC_Payment_Gateway {
	  ...
	  public function __construct() {
		  ...
		  $this->id                = 'base';
		  $this->icon              = apply_filters( 'woocommerce_base_icon', plugin_dir_url('') . '/Base/Base_Logo.png' );
		  $this->has_fields        = false;
		  $this->order_button_text = __( 'Proceed to payment', 'woocommerce' );
		  $this->service_directUrl = 'https://process.base.com/member/remote_charge.asp';
		  $this->service_hppUrl    = 'https://uiservices.base.com/hosted/';
		  $this->method_title      = __( 'base', 'woocommerce' );
		  $this->notify_url        = WC()->api_request_url( 'wc_gateway_base' );
	  ...
	  public function admin_options() {
		  ...
		  ?>
		  <h3><?php _e( 'base standard', 'woocommerce' ); ?></h3>
		  <p><?php _e( 'base works by sending the user to base to enter their payment information.', 'woocommerce' ); ?></p>
	  ...
	  function init_form_fields() {
		  ...
		  $this->form_fields = array(
			  'enabled' => array(
				  ...
				  'type'    => 'checkbox',
				  'label'   => __( 'Enable base standard', 'woocommerce' ),
				  'default' => 'yes'
			  ),
			  'title' => array(
			  ...
			  'description' => array(
				  ...
				  'default'     => __( 'Pay via base; you can pay with your credit card if you don\'t have a base account', 'woocommerce' )
			  ),
			  'merchantID' => array(
				  'title'       => __( 'base Merchant Number', 'woocommerce' ),
				  'type'        => 'number',
				  'description' => __( 'Your merchant account number at base.', 'woocommerce' ),
				  'default'     => '',
				  'desc_tip'    => true,
				  'placeholder' => '0000000'
			  ),
			  'securityKey' => array(
				  'title'       => __( 'base Security Key', 'woocommerce' ),
				  'type'        => 'text',
			  ...
			  'paymentaction' => array(
				  'title'       => __( 'base Action', 'woocommerce' ),
				  'type'        => 'select',
    ...
    function get_direct_args( $order ) {
	  ...
	  function receipt_page( $order_id ) {
		  echo '<p>' . __( 'Thank you - your order is now pending payment. You should be automatically redirected to base to make payment.', 'woocommerce' ) . '</p>';
		  ...
		  wc_enqueue_js( '
			  $.blockUI({
					  message: "' . esc_js( __( 'Thank you for your order. We are now redirecting you to base to make payment.', 'woocommerce' ) ) . '",
					  baseZ: 99999,
					  overlayCSS:
					  {
						  background: "#fff",
        ...
        '<form action="' . esc_url( $post_adr ) . '" method="post" id="payment_form" target="_top">
				  ...
				  <div class="payment_buttons">
					  <input type="submit" class="button alt" id="submit_payment_form" value="' . __( 'Pay via base', 'woocommerce' ) . '" /> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . __( 'Cancel order &amp; restore cart', 'woocommerce' ) . '</a>
				  </div>
    ...
    {
        $order->add_order_note( 'base ' . $modeText .  ' payment result :' . $replayCode);
      ...
      */
      ...
      if ($response_data && !empty( $response_data['replyCode'] ) ) {
        ...
        echo 'OK';
			  ...
			  wp_die( "base notify handler Request Failure", "base notify handler", array( 'response' => 200 ) );
		  }
	  }
